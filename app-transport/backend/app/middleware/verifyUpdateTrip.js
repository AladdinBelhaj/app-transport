...
checkDuplicateTrip = (req, res, next) => {
    const tripId = req.params.tripId; // Extract tripId from request parameters
    ...
    Trip.findOne({
        where: {
            [Op.and]: [
                {
                    id: {
                        [Op.ne]: tripId // Exclude the current trip ID
                    }
                },
                {
                    [Op.or]: [
...
const verifyUpdateTrip = {
    checkDuplicateTrip: checkDuplicateTrip,
};

module.exports = verifyUpdateTrip;
