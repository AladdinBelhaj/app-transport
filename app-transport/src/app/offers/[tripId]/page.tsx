...
interface TripData {
  id: string; // Ensure this matches the type used in your Map
  departCountry: string;
  departState: string;
  destCountry: string;
  desState: string;
...
interface OfferData {
  id: number;
  tripId: string; // Ensure this matches the type used in your Map
  userId: string;
  picture: string;
  pictureIds: string;
  objects: string;
...
const fetchData = async (tripId: string) => {
  try {
    const offerResponse = await axios.get<OfferData[]>(
      `${process.env.NEXT_PUBLIC_BACKEND_URL}/api/offers/${tripId}`,
    );
    const allTripsResponse = await axios.get<TripData[]>(
    ...
    return {
      props: {
        ...
        allTrips: allTripsResponse.data,
        users: userResponse.data,
      },
    };
  } catch (error) {
    console.error("Error fetching data:", error);
    return { props: { offers: [], allTrips: [], users: [] } };
  }
};

const ViewOffers = async ({ params }: { params: { tripId: string } }) => {
  const { tripId } = params;

  const { props } = await fetchData(tripId);
  const { offers, allTrips, users } = props;

  const tripDataMap = new Map<string, TripData>(
    allTrips.map((trip) => [trip.id, trip]),
  );
  ...
  const filteredOffers = offers.filter(
    (offer) => offer.tripId === tripId && offer.status === "accepted",
  );
  ...
  return (
    <DefaultLayout>
      ...
      {filteredOffers.map((offer) => {
        const tripData = tripDataMap.get(offer.tripId);
        const userData = userDataMap.get(offer.userId);
        const pictures = JSON.parse(offer.picture);
        const pictureIds = JSON.parse(offer.pictureIds);
        let totalWeight = 0;
